...
export  function FormEditCar(props: FormEditCarProps) {
    const {carData, setIsOpenDialog} = props
 ...
 const onSubmit = async (values: z.infer<typeof formSchema>) => {
    try {
        console.log("Enviando dados editados...", values);
      await axios.patch(`/api/car/${carData.id}/form`, values);
      toast.success("🚗 Veículo modificado com sucesso!");
      setIsOpenDialog(false);
      router.refresh(); // ✅ preferível ao reload()
    } catch (error) {
      console.error(error);
      toast.error("❌ Erro ao editar o veículo. Tente novamente.");
    }
  };
  ...
  return (
       <Form {...form}>
         ...
         >
           ...
           >
             {isSubmitting ? "Editando..." : "Editar veículo"}
           </Button>
         </form>
       </Form>
  )
